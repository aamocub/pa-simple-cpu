...
#(
    ...
    logic [31:0] instr_list[11] = {ill, add, mul, sub, addi, sub, lw, sw, addi, mul, mul};
    /*
    ...
    */
    ...
    logic [MEM_LEN-1:0][7:0] mem;  // memory array to store and read memory values
    ...
    initial begin
        if (DEBUG) begin
            for (int j = 0; j < MEM_LEN / 4; j = j + 1) begin
                mem[4*j+:4] = j;
            end
            ...
            for (int k = 0; k < $size(instr_list); k = k + 1) begin
                mem[PC_EXCEPTION_ADDR+4*k+:4] = instr_list[k+1];
            end
        end
    end
endmodule
